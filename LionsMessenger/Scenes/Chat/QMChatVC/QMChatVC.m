...
- (void)loadUser {
    ...
    NSInteger opponetUserID = [self.chatDialog opponentID];
    // get user from server
    @weakify(self);
    [[[QMCore instance].usersService getUserWithID:opponetUserID forceLoad:YES] continueWithBlock:^id _Nullable(BFTask<QBUUser *> * _Nonnull task) {
...
- (void)didPressSendButton:(UIButton *)__unused button
    ...
    QBChatMessage *message = [QMMessagesHelper chatMessageWithText:text
     ...
     pushText = [NSString stringWithFormat:@"%@ send you message  %@", senderDisplayName, text];
...
- (void)_sendMessage:(QBChatMessage *)message {
    ...
    [[[QMCore instance].chatService sendMessage:message toDialog:self.chatDialog saveToHistory:YES saveToStorage:YES] continueWithBlock:^id _Nullable(BFTask * _Nonnull __unused task) {
        ...
        NSInteger opponetUserID = [self.chatDialog opponentID];
        QBUUser *opponentUser = [QBUUser new];
        opponentUser.ID = opponetUserID;
    
        [QMNotification sendPushNotificationToUser:opponentUser withText:self->pushText];
